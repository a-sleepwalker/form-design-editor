  ...
  */
  ...
  fnCheckForm: function (type) {
    ...
    if (leipiEditor.hasContents()) {
      ...
      $.ajax({
        type: 'POST',
        url: '/index.jsp?s=/index/parse.html',
        // dataType : 'json',
...
var formPropConf = {
  ...
  eformTableid: '',
  userCn: '',
  FormDesignUrl: 'formdesign',
  newForm: function () {
    var self = this;
    var dialog = new UE.ui.Dialog({
      iframeUrl: UE.leipiFormDesignUrl + 'newForm/index.html',
      name: 'newform',
      editor: leipiEditor,
      title: '新建表单',
      ...
      buttons: [
        {
          ...
          onclick: function () {
            dialog.close(true);
            $('#propertyform').removeAttr('disabled');
          }
        },
        {
  ...
  propertyform: function () {
    var self = this;
    var dialog = new UE.ui.Dialog({
      // iframeUrl:leipiEditor.options.UEDITOR_HOME_URL + self.FormDesignUrl+'/newform.html',
      iframeUrl: UE.leipiFormDesignUrl + 'propertyForm/index.html',
      name: 'propertyform',
      editor: leipiEditor,
      title: '表单属性设置',
      cssRules: 'width:' + width + 'px;height:' + height + 'px;',
  ...
  selectuser: function () {
    ...
    var title = '选择用户';
    var dialog = new UE.ui.Dialog({
      iframeUrl: UE.leipiFormDesignUrl + 'selectUser/index.html',
      name: 'newform',
      editor: leipiEditor,
      title: title,
      cssRules: 'width:' + width + 'px;height:' + height + 'px;',
      buttons: [
        {
          ...
          onclick: function () {
            ...
            $('#openform').removeAttr('disabled');
            $('#saveform_').removeAttr('disabled');
          }
        },
        {
          className: 'edui-cancelbutton',
  ...
  openform: function () {
    ...
    var title = '打开表单';
    var dialog = new UE.ui.Dialog({
      // iframeUrl:path+'/servlet/FormDesignServ?action=GetAllForms&view=../admin/formdesign/js/ueditor/formdesign/opendialog.jsp',
      iframeUrl: UE.leipiFormDesignUrl + 'openForm/index.html',
      name: 'openform',
      editor: leipiEditor,
      title: title,
      cssRules: 'width:' + width + 'px;height:' + height + 'px;',
...
$(function () {
  $(window).on('storage', function () {
    if (sessionStorage.getItem('selectedUser')) {
      ...
      if (sessionStorage.getItem('current-form')) {
        if (!formFlag) {
        ...
        currentFormName = JSON.parse(sessionStorage.getItem('current-form')).NAME;
        $('#currentFormName').html(currentFormName);
      } else {
        $('#saveform_').off('click').addClass('disabled');
        $('#propertyform').off('click').addClass('disabled');
        formFlag = false;
      }
      $('#selectuser_alert').hide();
      $('#currentUser').html(sessionStorage.getItem('selectedUserName'));
    } else {
      $('#newform').off('click').addClass('disabled');
      $('#openform').off('click').addClass('disabled');
      $('#saveform_').off('click').addClass('disabled');
      $('#propertyform').off('click').addClass('disabled');
      userFlag = false;
      formFlag = false;
