              ...
              <el-table :data="optionList" highlightCurrentRow border
                ...
                <el-table-column label="选中" header-align="center" align="center" width="70px">
                  <template slot-scope="scope">
                    &nbsp;<el-radio v-model="formAttr.checked" :label="scope.row.webData.uniqueToken">&nbsp;
                  </el-radio>
                  </template>
                </el-table-column>
    ...
    methods: {
      fnAdd() {
        this.optionList.push({
          displayVal: '', saveVal: '', webData: {underModified: false, uniqueToken: new Date().getTime()}
        });
      },
      fnMove(steps) {
      ...
      if (UE.plugins[_this.pluginType].editdom) {
        ...
        for (let i = 0, l = inputTags.length; i < l; i++) {
          if (inputTags[i].checked) {
            _form.checked = inputTags[i].getAttribute('timestamp');
          }
          _this.optionList.push({
            displayVal: labelTags[i].innerHTML,
            saveVal: inputTags[i].value,
            checked: inputTags[i].checked,
            webData: {underModified: false, uniqueToken: inputTags[i].getAttribute('timestamp')}
          });
        }
      }
        ...
        if (!_node) {
          try {
            ...
            _this.optionList.forEach(v => {
              html += '<input name="' + fieldName + '" value="' + (v.saveVal === '' ? v.displayVal : v.saveVal) + '" ' + (v.webData.uniqueToken === _form.checked ? 'checked' : '') + ' type="radio" timestamp="' + v.webData.uniqueToken + '"/><label>' + v.displayVal + '</label>';
              //  竖排
